...
- **pi symlink binds** — `pi-bw` follows symlinks under `~/.pi/agent/` (recursively, up to 3 levels deep — e.g. `agents/*.md`, `extensions/<x>/*.ts`) and mounts their regular-file targets read-only (to support host-managed configs like `models.json` or a config repo symlinked into place). Because `~/.pi` is writable inside the sandbox, a compromised prior session could plant a symlink pointing at another same-user-readable file (e.g. `~/.aws/credentials`) to expose it read-only on a later launch — and with recursion the planted link can hide in a nested subdirectory instead of the conspicuous top level. The deny-files extension only pattern-matches common secret basenames on pi's tool calls and does not close this hole; bwrap remains the primary boundary. Audit `~/.pi/agent/` symlinks if the sandbox may have run untrusted output.
